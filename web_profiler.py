...
def run(site, path, browser):
    ...
    traffic_xml = raw_xml.replace('&', '&amp;').replace(
        '=""GET""', '="GET"').replace('=""POST""', '="POST"')  # workaround selenium bugs
    ...
    end_last_request_elapsed = get_elapsed_secs(
        start_first_request, end_last_request)
    end_first_request_elapsed = get_elapsed_secs(
        start_first_request, end_first_request)
...
class NetworkCapture(object):
    ...
    def convert_time(self, date_string):
        ...
        dt = datetime.strptime(
            ''.join(date_string.split(split_char)[:-1]), '%Y%m%dT%H:%M:%S.%f')
        return dt
