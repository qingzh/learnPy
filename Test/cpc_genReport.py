...
import sys
import json
from APITest.models.models import AttributeDict, TestRequest, APIData, _slots_class
...
class HttpServer(object):
    ...
    def __init__(self, session, server, username, password, headers=None):
        '''
        use `username` `password` to login `server`
        '''
    ...
    def server(self, value):
        self._server = _prepare_url(value)
    ...
    def prepare_cookies(self, uri=None):
        '''
        ...
        '''
        url = urlparse.urljoin(self.server, uri)
        ...
        assert res.status_code == 200, 'Post failed: %s' % res.url

        r = self.session.get(url)
        assert 'login' not in r.url.lower(), 'Login failed!'
